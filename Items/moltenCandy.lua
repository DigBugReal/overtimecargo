...
-- if actor:item_count(candy) <= 0 then return end
	-- if (math.random() <= 0.99) and (#Instance.find_all(moltenblob)) < 21 then -- wont spawn if 21 puddles max already exist, another performance cope
		-- for i = 0, 2, 1 do
			-- local blob = moltenblob:create(target.x, target.y)
			-- Instance.get_data(blob).direction = math.random(45, 135)
			-- Instance.get_data(blob).hsp = 16 --something changed abt spawning these now they just fall to the floor and keep falling infinitely
			-- Instance.get_data(blob).vsp = 20
			-- blob.gravity = 0.2 
...
-- -- Callback.add(Callback.ON_STAGE_START, function()
    -- -- for _, player in ipairs(Instance.find_all(gm.constants.oP)) do
        -- -- candy:create(player.x, player.y)
    -- -- end
-- -- end)