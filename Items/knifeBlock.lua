...
-- cut.required_interrupt_priority = ActorState.InterruptPriority.SKILL_INTERRUPT_PERIOD
...
-- Callback.add(state_cut.on_enter, function(actor, data)
	...
	-- actorData.Instance.get_data(swipes).parent = actor
...
-- Callback.add(state_cut.on_step, function(actor, data)
	...
	-- local actorData = Instance.get_data(actor)
	-- local swipeData = Instance.get_data(swipe)
	
	-- if actor:is_authority() and data.fired == 0 and Instance.get_data(actor).swipe.image_index >= 4 then
	...
	-- if not Instance.exists(actorData.swipes) then
		-- actor:skill_util_reset_activity_state()
	-- end
-- end)

-- Callback.add(state_cut.on_exit, function(actor, data)
	-- Instance.get_data(actor).swipe:destroy()
-- end)

-- Callback.add(state_cut.on_get_interrupt_priority, function(actor, data)
	-- if Instance.get_data(swipe).image_index >= 5 then
...
-- -- Callback.add(Callback.ON_STAGE_START, function()
    -- -- for _, player in ipairs(Instance.find_all(gm.constants.oP)) do
        -- -- block:create(player.x, player.y)
    -- -- end
-- -- end)